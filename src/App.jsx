  ...
  if (!supabaseLoadPromise) {
    supabaseLoadPromise = (async () => {
      try {
        ...
        // Ensure table exists is handled by user via SQL editor (one-time); we just use it.
...
function PropertyMap({ poolLen, poolWid, poolShape, poolColor, parcelData }) {
  ...
  const [googleKey, setGoogleKey] = useState(()=>{ try{return localStorage.getItem("pc_gmaps_key")||"";}catch{return "";} });
  const [keyInput, setKeyInput] = useState("");
...
function AIRenderingPanel({ bgPhoto, setBgPhoto, shape, poolColor, len, wid, finish, colorId, entries, hardscapes, dailyRenders=0, dailyLimit=10, onRenderComplete=()=>{} }) {
  ...
  const [apiKey, setApiKey] = useState(() => { try { return localStorage.getItem("xai_dev_key") || ""; } catch { return ""; } });
  const [keyInput, setKeyInput] = useState("");
...
function YardPlanner({ poolLen, poolWid, poolShape, poolColor, parcelData }) {
  ...
  const buildMapUrl = (addr, z) => {
    ...
    const gKey = (() => { try { return localStorage.getItem("pc_gmaps_key")||""; } catch { return ""; } })();
...
function SettingsScreen({ userMode, setUserMode, onSwitchMode }) {
  const [xaiKey, setXaiKey] = useState(()=>{ try{return localStorage.getItem("xai_dev_key")||"";}catch{return "";} });
  const [gmapsKey, setGmapsKey] = useState(()=>{ try{return localStorage.getItem("pc_gmaps_key")||"";}catch{return "";} });
  ...
  const renderNow = async () => {
    ...
    try {
      ...
      const resp = await fetch("https://api.x.ai/v1/images/edits", {
        ...
        body: JSON.stringify({ model: "grok-2-image-1212", prompt, image: { b64_json: b64, media_type: mediaType }, response_format: "b64_json", n: 1 }),
      });
      clearInterval(iv);
      if (!resp.ok) {
