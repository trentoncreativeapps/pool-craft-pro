...
function FeatureCard({ feature, active, onToggle }) {
  ...
  useEffect(() => {
    ...
    if(feature.id==="beach_entry"){
      ...
      ctx.fillStyle=c+"cc"; ctx.font="9px Inter,sans-serif"; ctx.textAlign="center"; ctx.fillText("âˆž Vanishing Edge",W*0.86,H*0.26);
      ...
      <div style={{ width:"100%", maxWidth:420 }}>
        ...
        <div style={{ background:"#111827", border:"1px solid #1e293b", borderRadius:20, padding:28, boxShadow:"0 20px 60px rgba(0,0,0,0.5)" }}>
          ...
          <div style={{ fontSize:13, color:"#64748b", marginBottom:24 }}>
            {mode==="login" ? "Welcome back â€” sign in to access your designs" : mode==="signup" ? "Start your free trial â€” no credit card needed" : "We'll send you a reset link"}
          </div>
          ...
          {error && <div style={{ marginBottom:14, padding:"10px 14px", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:10, fontSize:13, color:"#ef4444" }}>âš ï¸ {error}</div>}
          {success && <div style={{ marginBottom:14, padding:"10px 14px", background:"rgba(34,197,94,0.1)", border:"1px solid rgba(34,197,94,0.25)", borderRadius:10, fontSize:13, color:"#22c55e" }}>âœ… {success}</div>}
          ...
          <div style={{ marginTop:20, display:"flex", flexDirection:"column", gap:10, alignItems:"center" }}>
            ...
            {mode==="reset" && <button onClick={()=>{setMode("login");setError(null);}} style={{ background:"none", border:"none", color:"#64748b", fontSize:12, cursor:"pointer" }}>â† Back to sign in</button>}
          </div>
        </div>
...
export default function PoolCraftPro() {
  ...
  const showAuth = !user && !authedUser && !!SUPABASE_URL;

  const currentUser = user || authedUser; if (showAuth) return <AuthScreen onAuth={u => setAuthedUser(u)} />;
  ...
  const [savedToastMsg, setSavedToastMsg] = useState("âœ… Project saved!");
  ...
  // Demo mode â€” loads a sample pool so contractors can show the app during sales visits
  const activateDemo = () => {
    ...
    setProjectName("Demo â€” Lagoon Pool"); setClientName("Sample Client");
  ...
  const saveProject = async () => {
    ...
    setSavedToastMsg(cloudConfigured && !savedToCloud ? "âœ… Saved to this device (cloud sync had an issue)" : cloudConfigured ? "âœ… Saved & synced to the cloud" : "âœ… Project saved!");
      ...
      {showSaveDialog && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"#111827",border:"1px solid #1e293b",borderRadius:16,padding:24,width:"100%",maxWidth:380}}>
            <div style={{fontSize:15,fontWeight:800,color:"#e2e8f0",marginBottom:16}}>ðŸ’¾ {projectId?"Update":"Save"} Project</div>
          ...
          <div style={{background:"#111827",border:"1px solid #1e293b",borderRadius:16,padding:24,width:"100%",maxWidth:380}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <button onClick={async ()=>{ const action=unsavedConfirm; setUnsavedConfirm(null); await saveProject(); if(action) action(); }}
                style={{padding:"12px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#22c55e,#16a34a)",color:"white",fontWeight:800,fontSize:13,cursor:"pointer"}}>ðŸ’¾ Save, Then Continue</button>
      ...
      <div style={{background:"linear-gradient(135deg,#0a0f1e 0%,#0f1e3d 60%,#0a0f1e 100%)",padding:"14px 16px 0",borderBottom:"1px solid rgba(201,168,76,0.2)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:11,color:"#8a9ab5",display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",marginTop:2}}>
              <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:160}}>{clientName?`${clientName} Â· `:""}{projectName}</span>
              {(!projectId || isDirty) && (
          ...
          <div onClick={()=>setShowOnboarding(true)} style={{padding:"6px 10px",borderRadius:16,background:userMode==="homeowner"?"rgba(34,197,94,0.15)":userMode==="designer"?"rgba(201,168,76,0.15)":"rgba(74,122,181,0.2)",border:`1px solid ${userMode==="homeowner"?"rgba(34,197,94,0.3)":userMode==="designer"?"rgba(201,168,76,0.35)":"rgba(74,122,181,0.4)"}`,fontSize:10,color:userMode==="homeowner"?"#22c55e":userMode==="designer"?"#c9a84c":"#7ab0e8",fontWeight:700,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
            {userMode==="homeowner"?"ðŸ  HO":userMode==="designer"?"ðŸŽ¨ Design":"ðŸ‘· Pro"}
          </div>
        ...
        {demoMode && (
          <div style={{background:"rgba(245,158,11,0.15)",border:"1px solid rgba(245,158,11,0.4)",borderRadius:10,padding:"8px 12px",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div style={{fontSize:12,color:"#f59e0b",fontWeight:700}}>ðŸŽ¯ Demo Mode â€” Showing sample lagoon pool to client</div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
          <button onClick={()=>withUnsavedCheck(startNewProject)} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",color:"#c9a84c",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>âž• New</button>
          <button onClick={()=>{setSaveNameInput(projectName);setSaveClientInput(clientName||"");setSaveClientEmailInput(clientEmail||"");setSaveClientPhoneInput(clientPhone||"");setSaveShowContactFields(!!(clientEmail||clientPhone));setShowSaveDialog(true);}} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(34,197,94,0.12)",border:"1px solid rgba(34,197,94,0.3)",color:"#22c55e",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>ðŸ’¾ Save</button>
          <button onClick={()=>setShowProjects(true)} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(74,122,181,0.12)",border:"1px solid rgba(74,122,181,0.3)",color:"#7ab0e8",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>ðŸ“‚ Projects</button>
          <button onClick={exportPDF} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.25)",color:"#c9a84c",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>ðŸ“„ PDF</button>
          <button onClick={()=>setShowShare(true)} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(167,139,250,0.12)",border:"1px solid rgba(167,139,250,0.25)",color:"#a78bfa",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>ðŸ“¤ Share</button>
          {!demoMode && <button onClick={activateDemo} style={{padding:"7px 12px",minHeight:34,borderRadius:16,background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.25)",color:"#f59e0b",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>ðŸŽ¯ Demo</button>}
          {wishlist.length>0&&<div style={{padding:"7px 10px",minHeight:34,display:"flex",alignItems:"center",borderRadius:16,background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",fontSize:12,color:"#c9a84c",flexShrink:0}}>â¤ï¸ {wishlist.length}</div>}
        </div>
      ...
      <div style={{padding:"16px 14px",maxWidth:820,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {tab===0&&<>
          <div style={card}>
            ...
            <div style={{marginTop:10,fontSize:12,color:"#64748b"}}>ðŸ’¡ {POOL_SHAPES.find(s=>s.id===shape)?.desc}</div>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{marginTop:10,fontSize:12,color:"#64748b"}}>ðŸ’¡ {POOL_FINISHES.find(f=>f.id===finishId)?.desc}</div>
          </div>
          <div style={card}>
          ...
          <div style={card}>
            <div style={sectionTitle}>ðŸ§Š 3D Preview</div>
            <div style={{fontSize:12,color:"#64748b",marginBottom:10}}>An instant 3D model from your dimensions, shape, depth, and finish â€” no photo needed. Rotate it to check proportions before generating an AI rendering below.</div>
            <Pool3D poolLen={len} poolWid={wid} poolShape={shape} poolColor={poolColor.hex} depthId={depthId} entries={entries} finishId={finishId} />
          </div>

          <div style={card}>
            <div style={sectionTitle}>âœ¨ AI Pool Rendering</div>
          ...
          <div style={card}>
            <div style={sectionTitle}>ðŸ“ Property Lookup & Pool Placement</div>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={lookupAddress} disabled={parcelStatus==="loading"} style={{padding:"10px 16px",borderRadius:10,background:"linear-gradient(135deg,#06b6d4,#0284c7)",border:"none",color:"white",fontWeight:700,fontSize:13,cursor:"pointer",flexShrink:0,opacity:parcelStatus==="loading"?0.6:1}}>{parcelStatus==="loading"?"â³":"Search"}</button>
            </div>
            {parcelStatus==="found"&&parcelData&&<>
              <div style={{marginTop:12,background:"rgba(6,182,212,0.08)",border:"1px solid rgba(6,182,212,0.2)",borderRadius:12,padding:14}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div style={{fontSize:12,color:"#06b6d4",fontWeight:700}}>âœ… Parcel Found - {parcelData.address}</div>
                  <div style={{fontSize:10,padding:"2px 8px",borderRadius:20,background:parcelData.source==="regrid"?"rgba(34,197,94,0.15)":"rgba(245,158,11,0.15)",border:`1px solid ${parcelData.source==="regrid"?"rgba(34,197,94,0.3)":"rgba(245,158,11,0.3)"}`,color:parcelData.source==="regrid"?"#22c55e":"#f59e0b",fontWeight:700}}>{parcelData.source==="regrid"?"ðŸŸ¢ Live Regrid Data":"ðŸŸ¡ Estimated"}</div>
                </div>
              ...
              {showMap&&<div style={{marginTop:14}}>
                <div style={{fontSize:11,color:"#06b6d4",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:10}}>ðŸ—ºï¸ Interactive Property Map - Drag Pool to Place</div>
                <PropertyMap poolLen={len} poolWid={wid} poolShape={shape} poolColor={poolColor.hex} parcelData={parcelData}/>
                <div style={{marginTop:10,padding:"8px 12px",background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:8,fontSize:12,color:"#f59e0b"}}>âš ï¸ Setback data shown is for planning reference. Always verify with your local building department before permit submission.</div>
              </div>}
            </>}
          </div>

          <div style={card}>
            <div style={sectionTitle}>â˜ï¸ Cloud Sync</div>
            <CloudSyncPanel />
          </div>
        </>}
        ...
        {tab===1&&<>
          ...
          {Object.keys(entries).length > 0 && (
            <div style={{background:"linear-gradient(135deg,rgba(6,182,212,0.1),rgba(2,132,199,0.06))",border:"1px solid rgba(6,182,212,0.25)",borderRadius:12,padding:14,marginTop:4}}>
              <div style={{fontSize:12,color:"#06b6d4",fontWeight:700,marginBottom:8}}>âœ… Selected Features ({Object.keys(entries).length})</div>
        ...
        {tab===6&&<>
          <div style={card}>
            ...
            {[{label:"ðŸ”¥ Include Heater",key:"heater"},{label:"ðŸ’§ Water Features",key:"waterFeature"}].map(o=>(
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <span style={{fontSize:14}}>ðŸ§‚ Sanitization</span>
          ...
          {equipment.map(eq=>(
            <div key={eq.label} style={{background:"#111827",border:"1px solid #1e293b",borderRadius:14,overflow:"hidden"}}>
              ...
              {eq.asin && (<a href={pentairLink(eq.asin)} target="_blank" rel="noopener noreferrer" style={{display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 16px", background:"linear-gradient(135deg,rgba(255,153,0,0.15),rgba(255,120,0,0.08))", borderTop:"1px solid rgba(255,153,0,0.2)", textDecoration:"none", gap:10}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>ðŸ“¦</span><div><div style={{fontSize:12,fontWeight:700,color:"#ff9900"}}>Buy on Amazon</div><div style={{fontSize:10,color:"#64748b"}}>You earn {eq.earn} affiliate commission</div></div></div>
                <span style={{color:"#ff9900",fontSize:16}}>â†’</span>
              </a>)}
            </div>
          ))}
          <div style={{background:"rgba(255,153,0,0.06)",border:"1px solid rgba(255,153,0,0.2)",borderRadius:12,padding:12,textAlign:"center"}}>
            <div style={{fontSize:12,color:"#ff9900",fontWeight:700,marginBottom:3}}>ðŸ’° Earn 3-8% on every Pentair purchase</div>
        ...
        {tab===7&&<>
          <div style={card}>
            ...
            <div style={{display:"flex",gap:8}}>{[{id:"contractor",label:"ðŸ‘· Contractor"},{id:"diy",label:"ðŸ  Homeowner DIY"}].map(m=>(<button key={m.id} onClick={()=>setGuideMode(m.id)} style={{flex:1,padding:"10px 0",borderRadius:10,border:`2px solid ${guideMode===m.id?"#06b6d4":"#334155"}`,background:guideMode===m.id?"rgba(6,182,212,0.1)":"#1e293b",color:guideMode===m.id?"#06b6d4":"#94a3b8",cursor:"pointer",fontSize:13,fontWeight:700}}>{m.label}</button>))}</div>
            {guideMode==="diy"&&<div style={{marginTop:12,padding:"10px 12px",background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:10,fontSize:12,color:"#f59e0b"}}>âš ï¸ DIY pool building requires permits in all US states. Structural concrete & electrical must pass inspection.</div>}
          </div>
          {STEP_GUIDE.map((phase,i)=>(
            <div key={i} style={{background:"#111827",border:"1px solid #1e293b",borderRadius:14,overflow:"hidden"}}>
              <div style={{background:"linear-gradient(135deg,rgba(6,182,212,0.15),rgba(2,132,199,0.08))",padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{fontSize:11,color:"#64748b",background:"#1e293b",padding:"3px 10px",borderRadius:20}}>â± {phase.days}</div>
              </div>
        ...
        {tab===8&&<>
          <div style={{background:"linear-gradient(135deg,rgba(245,158,11,0.18),rgba(217,119,6,0.1))",border:"1px solid rgba(245,158,11,0.35)",borderRadius:16,padding:16}}>
            <div style={{fontSize:14,fontWeight:800,color:"#f59e0b"}}>ðŸ’° Affiliate Shopping - You Earn on Every Purchase</div>
          ...
          {wishlist.length>0&&(
            <div style={{background:"#111827",border:"1px solid rgba(245,158,11,0.3)",borderRadius:14,padding:16}}>
              <div style={{fontSize:13,fontWeight:700,color:"#f59e0b",marginBottom:10}}>â¤ï¸ Saved Items ({wishlist.length})</div>
